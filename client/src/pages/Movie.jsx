...
function Movie(){
    ...
    useEffect(()=>{
        document.title=`${movie.title} | Cine Hub`;
        return ()=>{
        ...
        <div className="movie-detail">
            <div className="movie-image">
                ...
                <div className="movie-actions">
                    <button className={`wat-btn ${watchlisted ? "active":""}`}  onClick={handleWatchList} title="watchlist">★</button>
                    <button className={`f-btn ${favorite?"active":""}`} id='fav' onClick={handleFavorite} title="favorite">♥</button>
                </div>
            </div>
            ...
            <div className="movie-data">
                ...
                {details && (
                <>
                    <p><span className="info">Genres:</span> {details.genres.map(g => g.name).join(', ')}</p>
                    <p><span className="info">Runtime:</span> {details.runtime} minutes</p>
                    <p><span className="info">Rating:</span> {details.vote_average.toFixed(1)} / 10</p>
                    {details.budget > 0 && <p><span className="info">Budget:</span> ${details.budget.toLocaleString()}</p>}
                    {details.revenue > 0 && <p><span className="info">Revenue:</span> ${details.revenue.toLocaleString()}</p>}
                </>
                )}
                ...
                <div>
                    {comments.map((comment)=>(
                        <div className="comment" key={comment.id}>
                            <strong className="comment-user">{comment.user_email}</strong>
                            {editCommentId===comment.id?(
                                <div className="comment-edit">
                                    ...
                                    <div className="comment-actions">
                                        <button id="edit-save" onClick={()=>handleEditSave(comment.id)}>Save</button>
                                ...
                                <div className="comment-content">
                                    <div>
                                        <p>{comment.comment}</p>
                                        <small className="comment-time">{new Date(comment.created_at).toLocaleString()}</small>
                                    </div>
